...
export default class GameScene extends Phaser.Scene {
  ...
  activeMonsters: Phaser.GameObjects.Group
  ...
  create() {
    ...
    this.activeMonsters = this.add.group({classType: Monster})
...
startMonster(monsters: Array<IMonster>) {
  ...
  if (monster) {
    new Promise((resolve) => {
      ...
      setTimeout(() => {
        const monsterInstance = new Monster(this, this.map.getRandomPath(), monster)
        .on("finish", () => {
          this.health = this.health - 10
          if (this.health <= 0) {
            this.game.events.emit('finish')
        ...
        this.activeMonsters.add(monsterInstance)
...
update(time, delta) {
    ...
    if (this.map.mapData.waves.length === 0 && this.activeMonsters.getLength()) {
      this.game.events.emit('finish');
    }
    if (this.activeMonsters.getLength() === 0 && Math.round((this.waveTimer.delay - this.waveTimer.getElapsed()) / 1000) === 0) {
      this.waveTimer = this.time.delayedCall(10000, () => { this.startWave() });
    }
    if (this.towers.length > 0 && this.activeMonsters.getLength() > 0) {
      this.towers.forEach(tower => {tower.update(time, delta, this.activeMonsters, this.bullets) });
    } 
